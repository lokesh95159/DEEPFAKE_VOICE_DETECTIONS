...
def classify_audio_clip(clip):
    ...
    state_dict = torch.load('/Users/karankilokesh/Downloads/final_year_project_allcodes/flask_practice/classifier.pth', map_location=torch.device('cpu'))
...
def index():
    if request.method == 'POST':
        ...
        if uploaded_file:
            ...
            if formatted_result<0 and formatted_result>1:
                if formatted_result<0.5:
                    formatted_result="Ai "
                else:
                    formatted_result="Human voice"
            if formatted_result < 1:
                if formatted_result < 50:
                    formatted_result = "Ai "
                else:
                    formatted_result = "Human voice"
            return render_template('index.html', result=formatted_result,per=per)
