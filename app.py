...
def load_audio(audiopath, sampling_rate=22000):
    if isinstance(audiopath, str):
        ...
        raise ValueError("Invalid audiopath format")
...
def classify_audio_clip(clip):
    ...
    state_dict = torch.load('/Users/karankilokesh/Downloads/final_year_project_allcodes/flask_code_video/classifier.pth', map_location=torch.device('cpu'))
...
def index():
    if request.method == 'POST':
        ...
        if uploaded_file:
            ...
            audio_clip = load_audio(io.BytesIO(audio_data))
            result = classify_audio_clip(audio_clip)
            result = result.item()
            formatted_result = int(result * 100)
            formatted_result = round(result * 100, 2)
            per=formatted_result
            formatted_result="AI Generated"
            text=""
            if per==0:
                text="So the uploaded audio is real"
            elif per>0 and per<50:
                text="So the uploaded audio is manipulated"
            elif per>=50:
                text="So the uploaded audio is deepfaked"
            return render_template('index.html', result=formatted_result,per=per,text=text)
